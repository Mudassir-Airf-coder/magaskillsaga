...
2. **Reach Reddit another way; fetch endpoints for the rest.** For Reddit, the thing that holds up best is a real browser (a Playwright-style tool) pointed straight at the actual `reddit.com` thread URL. A real browser running on a normal residential connection passes Reddit's bot challenge on its own and renders the whole page, comments included, which a plain URL-fetcher can't do. Two things get in the way, and both have the same fallback: some agent environments put reddit.com on a policy blocklist so their *built-in* browser refuses it (Claude Code's browser pane does this, though a separate Playwright tool in the same environment is not blocked), and Reddit occasionally escalates to a full CAPTCHA on heavy repeated hits. When either happens, point the same real browser at a Redlib mirror instead, an alternative Reddit front-end (e.g. `redlib.privacyredirect.com`) that fetches Reddit from an allowed address and serves plain HTML; a mirror is on nobody's reddit.com blocklist. Mirrors are volatile, so keep a rotating list (`l.opnxng.com` and whatever the live redlib instance list shows, since instances die often) and fall through it; the mirror has its own bot challenge that, again, only a real browser passes. Don't reach for a Google API here: Google closed its Custom Search JSON API to new users and is retiring it in 2027, so it returns a permission error no matter how it's configured. The only keyed option left is a paid third-party search API, and the one worth setting up is Serper.dev (see "The Serper.dev upgrade" right below the ladder). For review sites: try the cached or mobile version of the page.
